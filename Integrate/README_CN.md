# 警告
目前，手动融合存在一些问题，我们建议内核开发者等待ReKernel对手动融合完善后再将ReKernel融合进你的内核。
...
首先 你下载并将Kernel Modifier放入内核源代码的根目录中 然后使用17及以上版本的Java运行Kernel Modifier, 不出意外的话 Modifier 会自动修改内核源码并将ReKernel插入到你的内核当中 接下来你只需要重新编译一次内核即可
